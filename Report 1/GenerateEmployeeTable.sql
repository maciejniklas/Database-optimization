...
/* SEQUENCES */
CREATE SEQUENCE TempFirstNameSEQ START WITH 1;
CREATE SEQUENCE TempLastNameSEQ START WITH 1;

/* TRIGGERS */
CREATE OR REPLACE TRIGGER IndexerTempFirstNameTRIG BEFORE INSERT ON TempFirstName 
FOR EACH ROW 
BEGIN
  SELECT TempFirstNameSEQ.NEXTVAL INTO :new.tempID FROM dual;
END;
/

CREATE OR REPLACE TRIGGER IndexerTempLastNameTRIG BEFORE INSERT ON TempLastName 
FOR EACH ROW 
BEGIN
  SELECT TempLastNameSEQ.NEXTVAL INTO :new.tempID FROM dual;
END;
/
...
BEGIN
  ...
  FOR i IN 1..15000 LOOP    
    IF MOD(LocalAddressSEQ.nextval, 10) = 1 THEN
        INSERT INTO Employee(addressID, breweryID, firstName, lastName, birthDate, salary) VALUES
        (
            (LocalAddressSEQ.currval),
            (LocalBrewerySEQ.nextval),
            (SELECT tempValue FROM TempFirstName WHERE tempID = TRUNC(DBMS_RANDOM.VALUE(1, (SELECT COUNT(*) FROM TempFirstName) + 1))),
            (SELECT tempValue FROM TempLastName WHERE tempID = TRUNC(DBMS_RANDOM.VALUE(1, (SELECT COUNT(*) FROM TempLastName) + 1))),
            (TRUNC(SYSDATE - DBMS_RANDOM.value(366 * min_age, 366 * max_age))),
...
DROP SEQUENCE TempLastNameSEQ;
DROP SEQUENCE TempFirstNameSEQ;
DROP SEQUENCE LocalAddressSEQ;
